#!/usr/bin/env python3
"""
AGENTS-OS v6.5 SWARM - Claude Code Project Bootstrapper
Wariant bootstrap.py dedykowany dla VS Code + Claude Code Extension.

Dodatkowe kroki (vs bootstrap.py):
  - Kopiuje vault/CLAUDE.md → project root (Builder manifest)
  - Kopiuje vault/.claude/commands/ → .claude/commands/ (slash commands)
  - Ustawia claude-code jako primary builder w agents.yaml
  - Weryfikuje że kod otwiera się w VS Code (nie Antigravity IDE)

Wypisuje __PROJECT_DIR__:<ścieżka> jako ostatnią linię (używana przez os-init-claude).
"""
import os
import sys
import shutil
...
print(f"🚀 INICJACJA AGENTS-OS v6.0 SWARM W: {TARGET_DIR}")
...
# --------------------------------------------------------------------------- #
# 2b. Claude Code extras: CLAUDE.md + .claude/commands/ (Builder manifest)
# --------------------------------------------------------------------------- #
print("🤖 Konfiguracja Claude Code Builder (CLAUDE.md + slash commands)...")

# Źródła: najpierw lokalny vault projektu (skopiowany w kroku 2), potem globalny VAULT_DIR
claude_md_sources = [
    os.path.join(TARGET_DIR, "vault", "CLAUDE.md"),
    os.path.join(VAULT_DIR, "CLAUDE.md"),
]
claude_commands_sources = [
    os.path.join(TARGET_DIR, "vault", ".claude", "commands"),
    os.path.join(VAULT_DIR, ".claude", "commands"),
]

# Kopiuj CLAUDE.md do root projektu
claude_md_dst = os.path.join(TARGET_DIR, "CLAUDE.md")
if not os.path.exists(claude_md_dst):
    for src in claude_md_sources:
        if os.path.exists(src):
            shutil.copy2(src, claude_md_dst)
            print(f"   ✅ CLAUDE.md skopiowany z {src}")
            break
    else:
        # Fallback: wygeneruj minimalny CLAUDE.md
...
**Role:** Builder (VS Code Claude Code Extension)  
**Swarm Protocol:** AGENTS-OS v6.5 Swarm Edition  

## Mandatory Rules

1. ALWAYS create a git worktree before implementation (`/worktree-init`).
2. Generate handshake JSON after task completion (`/handshake`).
3. Run QA gate before PR (`/qa-gate`).
4. Commits must use `SWARM_ROLE=builder` env.
5. Reference: `SDLC.md` for full workflow.
""")
        print("   ⚠️  CLAUDE.md wygenerowany (minimalny — brak vault source).")
else:
    print("   ℹ️  CLAUDE.md już istnieje, pomijam.")

# Kopiuj .claude/commands/ do root projektu
claude_commands_dst = os.path.join(TARGET_DIR, ".claude", "commands")
os.makedirs(claude_commands_dst, exist_ok=True)
commands_installed = False
for src_dir in claude_commands_sources:
    if os.path.isdir(src_dir):
        for cmd_file in os.listdir(src_dir):
            src_file = os.path.join(src_dir, cmd_file)
            dst_file = os.path.join(claude_commands_dst, cmd_file)
            if not os.path.exists(dst_file) and os.path.isfile(src_file):
                shutil.copy2(src_file, dst_file)
        print(f"   ✅ Slash commands skopiowane: {claude_commands_dst}")
        commands_installed = True
        break
if not commands_installed:
    print("   ⚠️  Brak slash commands w vault — folder .claude/commands/ pusty.")

# Zaktualizuj agents.yaml: oznacz claude-code jako primary builder (jeśli plik istnieje)
agents_yaml_path = os.path.join(TARGET_DIR, "agents.yaml")
if os.path.exists(agents_yaml_path):
    try:
        with open(agents_yaml_path, "r") as f:
            content = f.read()
        # Dodaj notę do sekcji claude-code jeśli brak
        if "primary_builder: true" not in content and "claude-code:" in content:
            content = content.replace(
                "  claude-code:",
                "  claude-code:  # PRIMARY BUILDER (os-init-claude)\n  # primary_builder: true"
            )
            with open(agents_yaml_path, "w") as f:
                f.write(content)
            print("   ✅ agents.yaml: claude-code oznaczony jako primary builder.")
...
if repo.is_dirty(untracked_files=True):
    ...
    try:
        ...
        repo.git.commit("-m", "init: agents-os v6.5 swarm claude-code bootstrap", env=env)
        print("   ✅ Commit gotowy.")
    except Exception as e:
...
if not repo_exists and token:
    try:
        ...
        gh_repo = user.create_repo(
            name=project_name,
            private=False,
            description=f"AGENTS-OS v6.5 Swarm (Claude Code) — {project_name}"
        )
...
print(f"\n✨ AGENTS-OS v6.5 Swarm (Claude Code) — projekt GOTOWY.")
