# MCP
Source URL: https://antigravity.google/docs/cli/mcp#antigravity-cli
...
Modify the mcp_config.json file with your custom MCP server configuration.
...
In Python applications built using the Antigravity SDK, MCP servers (stdio, SSE, or HTTP) can be connected programmatically under a unified execution pipeline alongside built-in tools and custom Python functions.
...
MCP Configuration Structurelink
...
Navigate to the Customizations tab and click the Authenticate button next to the server.
