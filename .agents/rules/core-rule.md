...
## Rule 4: System Version Consistency (v6.0+)
- Whenever any system improvements, bug fixes, or new features are introduced to `agents-os-core`, the system version number (e.g. `v6.0`) must be updated across all files of the project, including:
  1. Bootstrapper scripts (`bootstrap.py`) and initializers (`os-init`).
  2. Installation scripts (`INSTALL.sh`) and default template directory names (e.g. `v6.0-swarm`).
