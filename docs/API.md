...
```bash
os-upgrade-project [target-directory] [--update-skills]
```

#### Parametry Wejściowe:
- `[target-directory]` *(string, opcjonalny)*: Ścieżka do aktualizowanego projektu. Domyślnie bieżący katalog roboczy (`pwd`).
- `--update-skills`, `-u` *(opcjonalny)*: Wymusza aktualizację / nadpisanie istniejących skilli do najnowszej wersji z Vault (zamiast dociągania tylko brakujących).
...
4. **Symlinki Claude Code**: Tworzy dowiązania `ln -sfn` ze wszystkich `.agents/skills/*` do `.claude/skills/` (63+ skilli, w tym 41 skilli Open-Mercato).
